...
class AgentCreate(StrictModel):
    ...
    def validate_actions(cls, value: list[str]) -> list[str]:
        ...
        if not normalized:
            raise ValueError("allowed_actions must contain at least one non-empty action")
...
class BudgetUpsert(StrictModel):
    ...
    def normalize_currency(cls, value: str) -> str:
        ...
        if not normalized.isalpha():
            raise ValueError("currency must contain only alphabetic ISO-4217 characters")
        return normalized
...
class ExecutionIntent(StrictModel):
    ...
    requested_amount: Decimal | None = Field(default=None, gt=0, max_digits=18, decimal_places=4)
